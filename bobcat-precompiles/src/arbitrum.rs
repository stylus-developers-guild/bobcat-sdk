...
use bobcat_cd::{
    const_keccak_sel, leftpad_addr, leftpad_bool, leftpad_u8, leftpad_u16, leftpad_u32,
    leftpad_usize,
};
...
const fn leftpad_u64(x: u64) -> [u8; 32] {
    concat_arrays!([0u8; 32 - 8], x.to_be_bytes())
}

/// Address of the `arb_sys` precompile.
const ADDR_ARB_SYS: [u8; 20] = U::from_u32(100).const_addr();
/// Address of the `arb_info` precompile.
const ADDR_ARB_INFO: [u8; 20] = U::from_u32(101).const_addr();
/// Address of the `arb_address_table` precompile.
const ADDR_ARB_ADDRESS_TABLE: [u8; 20] = U::from_u32(102).const_addr();
/// Address of the `arb_function_table` precompile.
const ADDR_ARB_FUNCTION_TABLE: [u8; 20] = U::from_u32(104).const_addr();
/// Address of the `arbos_test` precompile.
const ADDR_ARBOS_TEST: [u8; 20] = U::from_u32(105).const_addr();
/// Address of the `arb_owner_public` precompile.
const ADDR_ARB_OWNER_PUBLIC: [u8; 20] = U::from_u32(107).const_addr();
/// Address of the `arb_gas_info` precompile.
const ADDR_ARB_GAS_INFO: [u8; 20] = U::from_u32(108).const_addr();
/// Address of the `arb_aggregator` precompile.
const ADDR_ARB_AGGREGATOR: [u8; 20] = U::from_u32(109).const_addr();
/// Address of the `arb_retryable_tx` precompile.
const ADDR_ARB_RETRYABLE_TX: [u8; 20] = U::from_u32(110).const_addr();
/// Address of the `arb_statistics` precompile.
const ADDR_ARB_STATISTICS: [u8; 20] = U::from_u32(111).const_addr();
/// Address of the `arb_owner` precompile.
const ADDR_ARB_OWNER: [u8; 20] = U::from_u32(112).const_addr();
/// Address of the `arb_wasm` precompile.
const ADDR_ARB_WASM: [u8; 20] = U::from_u32(113).const_addr();
/// Address of the `arb_wasm_cache` precompile.
const ADDR_ARB_WASM_CACHE: [u8; 20] = U::from_u32(114).const_addr();
/// Address of the `arb_native_token_manager` precompile.
const ADDR_ARB_NATIVE_TOKEN_MANAGER: [u8; 20] = U::from_u32(115).const_addr();
/// Address of the `arb_debug` precompile.
const ADDR_ARB_DEBUG: [u8; 20] = U::from_u32(255).const_addr();

const SEL_ARB_SYS_ARBBLOCKNUMBER: [u8; 4] = const_keccak_sel(b"arbBlockNumber()");
const SEL_ARB_SYS_ARBBLOCKHASH: [u8; 4] = const_keccak_sel(b"arbBlockHash(uint256)");
const SEL_ARB_SYS_ARBCHAINID: [u8; 4] = const_keccak_sel(b"arbChainID()");
const SEL_ARB_SYS_ARBOSVERSION: [u8; 4] = const_keccak_sel(b"arbOSVersion()");
const SEL_ARB_SYS_GETSTORAGEGASAVAILABLE: [u8; 4] = const_keccak_sel(b"getStorageGasAvailable()");
const SEL_ARB_SYS_ISTOPLEVELCALL: [u8; 4] = const_keccak_sel(b"isTopLevelCall()");
const SEL_ARB_SYS_MAPL1SENDERCONTRACTADDRESSTOL2ALIAS: [u8; 4] =
    const_keccak_sel(b"mapL1SenderContractAddressToL2Alias(address,address)");
const SEL_ARB_SYS_WASMYCALLERSADDRESSALIASED: [u8; 4] =
    const_keccak_sel(b"wasMyCallersAddressAliased()");
const SEL_ARB_SYS_MYCALLERSADDRESSWITHOUTALIASING: [u8; 4] =
    const_keccak_sel(b"myCallersAddressWithoutAliasing()");
const SEL_ARB_SYS_WITHDRAWETH: [u8; 4] = const_keccak_sel(b"withdrawEth(address)");
const SEL_ARB_SYS_SENDTXTOL1: [u8; 4] = const_keccak_sel(b"sendTxToL1(address,bytes)");
const SEL_ARB_SYS_SENDMERKLETREESTATE: [u8; 4] = const_keccak_sel(b"sendMerkleTreeState()");

const SEL_ARB_INFO_GETBALANCE: [u8; 4] = const_keccak_sel(b"getBalance(address)");
const SEL_ARB_INFO_GETCODE: [u8; 4] = const_keccak_sel(b"getCode(address)");

const SEL_ARB_ADDRESS_TABLE_ADDRESSEXISTS: [u8; 4] = const_keccak_sel(b"addressExists(address)");
const SEL_ARB_ADDRESS_TABLE_COMPRESS: [u8; 4] = const_keccak_sel(b"compress(address)");
const SEL_ARB_ADDRESS_TABLE_DECOMPRESS: [u8; 4] = const_keccak_sel(b"decompress(bytes,uint256)");
const SEL_ARB_ADDRESS_TABLE_LOOKUP: [u8; 4] = const_keccak_sel(b"lookup(address)");
const SEL_ARB_ADDRESS_TABLE_LOOKUPINDEX: [u8; 4] = const_keccak_sel(b"lookupIndex(uint256)");
const SEL_ARB_ADDRESS_TABLE_REGISTER: [u8; 4] = const_keccak_sel(b"register(address)");
const SEL_ARB_ADDRESS_TABLE_SIZE: [u8; 4] = const_keccak_sel(b"size()");

const SEL_ARB_FUNCTION_TABLE_UPLOAD: [u8; 4] = const_keccak_sel(b"upload(bytes)");
const SEL_ARB_FUNCTION_TABLE_SIZE: [u8; 4] = const_keccak_sel(b"size(address)");
const SEL_ARB_FUNCTION_TABLE_GET: [u8; 4] = const_keccak_sel(b"get(address,uint256)");

const SEL_ARBOS_TEST_BURNARBGAS: [u8; 4] = const_keccak_sel(b"burnArbGas(uint256)");

const SEL_ARB_OWNER_PUBLIC_ISCHAINOWNER: [u8; 4] = const_keccak_sel(b"isChainOwner(address)");
const SEL_ARB_OWNER_PUBLIC_RECTIFYCHAINOWNER: [u8; 4] =
    const_keccak_sel(b"rectifyChainOwner(address)");
const SEL_ARB_OWNER_PUBLIC_GETALLCHAINOWNERS: [u8; 4] = const_keccak_sel(b"getAllChainOwners()");
const SEL_ARB_OWNER_PUBLIC_GETNATIVETOKENMANAGEMENTFROM: [u8; 4] =
    const_keccak_sel(b"getNativeTokenManagementFrom()");
const SEL_ARB_OWNER_PUBLIC_ISNATIVETOKENOWNER: [u8; 4] =
    const_keccak_sel(b"isNativeTokenOwner(address)");
const SEL_ARB_OWNER_PUBLIC_GETALLNATIVETOKENOWNERS: [u8; 4] =
    const_keccak_sel(b"getAllNativeTokenOwners()");
const SEL_ARB_OWNER_PUBLIC_GETTRANSACTIONFILTERINGFROM: [u8; 4] =
    const_keccak_sel(b"getTransactionFilteringFrom()");
const SEL_ARB_OWNER_PUBLIC_ISTRANSACTIONFILTERER: [u8; 4] =
    const_keccak_sel(b"isTransactionFilterer(address)");
const SEL_ARB_OWNER_PUBLIC_GETALLTRANSACTIONFILTERERS: [u8; 4] =
    const_keccak_sel(b"getAllTransactionFilterers()");
const SEL_ARB_OWNER_PUBLIC_GETFILTEREDFUNDSRECIPIENT: [u8; 4] =
    const_keccak_sel(b"getFilteredFundsRecipient()");
const SEL_ARB_OWNER_PUBLIC_GETNETWORKFEEACCOUNT: [u8; 4] =
    const_keccak_sel(b"getNetworkFeeAccount()");
const SEL_ARB_OWNER_PUBLIC_GETINFRAFEEACCOUNT: [u8; 4] = const_keccak_sel(b"getInfraFeeAccount()");
const SEL_ARB_OWNER_PUBLIC_GETBROTLICOMPRESSIONLEVEL: [u8; 4] =
    const_keccak_sel(b"getBrotliCompressionLevel()");
const SEL_ARB_OWNER_PUBLIC_GETPARENTGASFLOORPERTOKEN: [u8; 4] =
    const_keccak_sel(b"getParentGasFloorPerToken()");
const SEL_ARB_OWNER_PUBLIC_GETSCHEDULEDUPGRADE: [u8; 4] =
    const_keccak_sel(b"getScheduledUpgrade()");
const SEL_ARB_OWNER_PUBLIC_ISCALLDATAPRICEINCREASEENABLED: [u8; 4] =
    const_keccak_sel(b"isCalldataPriceIncreaseEnabled()");
const SEL_ARB_OWNER_PUBLIC_GETCOLLECTTIPS: [u8; 4] = const_keccak_sel(b"getCollectTips()");
const SEL_ARB_OWNER_PUBLIC_GETMAXSTYLUSCONTRACTFRAGMENTS: [u8; 4] =
    const_keccak_sel(b"getMaxStylusContractFragments()");

const SEL_ARB_GAS_INFO_GETPRICESINWEIWITHAGGREGATOR: [u8; 4] =
    const_keccak_sel(b"getPricesInWeiWithAggregator(address)");
const SEL_ARB_GAS_INFO_GETPRICESINWEI: [u8; 4] = const_keccak_sel(b"getPricesInWei()");
const SEL_ARB_GAS_INFO_GETPRICESINARBGASWITHAGGREGATOR: [u8; 4] =
    const_keccak_sel(b"getPricesInArbGasWithAggregator(address)");
const SEL_ARB_GAS_INFO_GETPRICESINARBGAS: [u8; 4] = const_keccak_sel(b"getPricesInArbGas()");
const SEL_ARB_GAS_INFO_GETGASACCOUNTINGPARAMS: [u8; 4] =
    const_keccak_sel(b"getGasAccountingParams()");
const SEL_ARB_GAS_INFO_GETMAXTXGASLIMIT: [u8; 4] = const_keccak_sel(b"getMaxTxGasLimit()");
const SEL_ARB_GAS_INFO_GETMINIMUMGASPRICE: [u8; 4] = const_keccak_sel(b"getMinimumGasPrice()");
const SEL_ARB_GAS_INFO_GETL1BASEFEEESTIMATE: [u8; 4] = const_keccak_sel(b"getL1BaseFeeEstimate()");
const SEL_ARB_GAS_INFO_GETL1BASEFEEESTIMATEINERTIA: [u8; 4] =
    const_keccak_sel(b"getL1BaseFeeEstimateInertia()");
const SEL_ARB_GAS_INFO_GETL1REWARDRATE: [u8; 4] = const_keccak_sel(b"getL1RewardRate()");
const SEL_ARB_GAS_INFO_GETL1REWARDRECIPIENT: [u8; 4] = const_keccak_sel(b"getL1RewardRecipient()");
const SEL_ARB_GAS_INFO_GETL1GASPRICEESTIMATE: [u8; 4] =
    const_keccak_sel(b"getL1GasPriceEstimate()");
const SEL_ARB_GAS_INFO_GETCURRENTTXL1GASFEES: [u8; 4] =
    const_keccak_sel(b"getCurrentTxL1GasFees()");
const SEL_ARB_GAS_INFO_GETGASBACKLOG: [u8; 4] = const_keccak_sel(b"getGasBacklog()");
const SEL_ARB_GAS_INFO_GETPRICINGINERTIA: [u8; 4] = const_keccak_sel(b"getPricingInertia()");
const SEL_ARB_GAS_INFO_GETGASBACKLOGTOLERANCE: [u8; 4] =
    const_keccak_sel(b"getGasBacklogTolerance()");
const SEL_ARB_GAS_INFO_GETL1PRICINGSURPLUS: [u8; 4] = const_keccak_sel(b"getL1PricingSurplus()");
const SEL_ARB_GAS_INFO_GETPERBATCHGASCHARGE: [u8; 4] = const_keccak_sel(b"getPerBatchGasCharge()");
const SEL_ARB_GAS_INFO_GETAMORTIZEDCOSTCAPBIPS: [u8; 4] =
    const_keccak_sel(b"getAmortizedCostCapBips()");
const SEL_ARB_GAS_INFO_GETL1FEESAVAILABLE: [u8; 4] = const_keccak_sel(b"getL1FeesAvailable()");
const SEL_ARB_GAS_INFO_GETL1PRICINGEQUILIBRATIONUNITS: [u8; 4] =
    const_keccak_sel(b"getL1PricingEquilibrationUnits()");
const SEL_ARB_GAS_INFO_GETLASTL1PRICINGUPDATETIME: [u8; 4] =
    const_keccak_sel(b"getLastL1PricingUpdateTime()");
const SEL_ARB_GAS_INFO_GETL1PRICINGFUNDSDUEFORREWARDS: [u8; 4] =
    const_keccak_sel(b"getL1PricingFundsDueForRewards()");
const SEL_ARB_GAS_INFO_GETL1PRICINGUNITSSINCEUPDATE: [u8; 4] =
    const_keccak_sel(b"getL1PricingUnitsSinceUpdate()");
const SEL_ARB_GAS_INFO_GETLASTL1PRICINGSURPLUS: [u8; 4] =
    const_keccak_sel(b"getLastL1PricingSurplus()");
const SEL_ARB_GAS_INFO_GETMAXBLOCKGASLIMIT: [u8; 4] = const_keccak_sel(b"getMaxBlockGasLimit()");
const SEL_ARB_GAS_INFO_GETGASPRICINGCONSTRAINTS: [u8; 4] =
    const_keccak_sel(b"getGasPricingConstraints()");
const SEL_ARB_GAS_INFO_GETMULTIGASPRICINGCONSTRAINTS: [u8; 4] =
    const_keccak_sel(b"getMultiGasPricingConstraints()");
const SEL_ARB_GAS_INFO_GETMULTIGASBASEFEE: [u8; 4] = const_keccak_sel(b"getMultiGasBaseFee()");

const SEL_ARB_AGGREGATOR_GETPREFERREDAGGREGATOR: [u8; 4] =
    const_keccak_sel(b"getPreferredAggregator(address)");
const SEL_ARB_AGGREGATOR_GETDEFAULTAGGREGATOR: [u8; 4] =
    const_keccak_sel(b"getDefaultAggregator()");
const SEL_ARB_AGGREGATOR_GETBATCHPOSTERS: [u8; 4] = const_keccak_sel(b"getBatchPosters()");
const SEL_ARB_AGGREGATOR_ADDBATCHPOSTER: [u8; 4] = const_keccak_sel(b"addBatchPoster(address)");
const SEL_ARB_AGGREGATOR_GETFEECOLLECTOR: [u8; 4] = const_keccak_sel(b"getFeeCollector(address)");
const SEL_ARB_AGGREGATOR_SETFEECOLLECTOR: [u8; 4] =
    const_keccak_sel(b"setFeeCollector(address,address)");
const SEL_ARB_AGGREGATOR_GETTXBASEFEE: [u8; 4] = const_keccak_sel(b"getTxBaseFee(address)");
const SEL_ARB_AGGREGATOR_SETTXBASEFEE: [u8; 4] = const_keccak_sel(b"setTxBaseFee(address,uint256)");

const SEL_ARB_RETRYABLE_TX_REDEEM: [u8; 4] = const_keccak_sel(b"redeem(bytes32)");
const SEL_ARB_RETRYABLE_TX_GETLIFETIME: [u8; 4] = const_keccak_sel(b"getLifetime()");
const SEL_ARB_RETRYABLE_TX_GETTIMEOUT: [u8; 4] = const_keccak_sel(b"getTimeout(bytes32)");
const SEL_ARB_RETRYABLE_TX_KEEPALIVE: [u8; 4] = const_keccak_sel(b"keepalive(bytes32)");
const SEL_ARB_RETRYABLE_TX_GETBENEFICIARY: [u8; 4] = const_keccak_sel(b"getBeneficiary(bytes32)");
const SEL_ARB_RETRYABLE_TX_CANCEL: [u8; 4] = const_keccak_sel(b"cancel(bytes32)");
const SEL_ARB_RETRYABLE_TX_GETCURRENTREDEEMER: [u8; 4] = const_keccak_sel(b"getCurrentRedeemer()");
const SEL_ARB_RETRYABLE_TX_SUBMITRETRYABLE: [u8; 4] = const_keccak_sel(b"submitRetryable(bytes32,uint256,uint256,uint256,uint256,uint64,uint256,address,address,address,bytes)");

const SEL_ARB_STATISTICS_GETSTATS: [u8; 4] = const_keccak_sel(b"getStats()");

const SEL_ARB_OWNER_ADDCHAINOWNER: [u8; 4] = const_keccak_sel(b"addChainOwner(address)");
const SEL_ARB_OWNER_REMOVECHAINOWNER: [u8; 4] = const_keccak_sel(b"removeChainOwner(address)");
const SEL_ARB_OWNER_ISCHAINOWNER: [u8; 4] = const_keccak_sel(b"isChainOwner(address)");
const SEL_ARB_OWNER_GETALLCHAINOWNERS: [u8; 4] = const_keccak_sel(b"getAllChainOwners()");
const SEL_ARB_OWNER_SETNATIVETOKENMANAGEMENTFROM: [u8; 4] =
    const_keccak_sel(b"setNativeTokenManagementFrom(uint64)");
const SEL_ARB_OWNER_SETTRANSACTIONFILTERINGFROM: [u8; 4] =
    const_keccak_sel(b"setTransactionFilteringFrom(uint64)");
const SEL_ARB_OWNER_ADDNATIVETOKENOWNER: [u8; 4] =
    const_keccak_sel(b"addNativeTokenOwner(address)");
const SEL_ARB_OWNER_REMOVENATIVETOKENOWNER: [u8; 4] =
    const_keccak_sel(b"removeNativeTokenOwner(address)");
const SEL_ARB_OWNER_ISNATIVETOKENOWNER: [u8; 4] = const_keccak_sel(b"isNativeTokenOwner(address)");
const SEL_ARB_OWNER_GETALLNATIVETOKENOWNERS: [u8; 4] =
    const_keccak_sel(b"getAllNativeTokenOwners()");
const SEL_ARB_OWNER_ADDTRANSACTIONFILTERER: [u8; 4] =
    const_keccak_sel(b"addTransactionFilterer(address)");
const SEL_ARB_OWNER_REMOVETRANSACTIONFILTERER: [u8; 4] =
    const_keccak_sel(b"removeTransactionFilterer(address)");
const SEL_ARB_OWNER_ISTRANSACTIONFILTERER: [u8; 4] =
    const_keccak_sel(b"isTransactionFilterer(address)");
const SEL_ARB_OWNER_GETALLTRANSACTIONFILTERERS: [u8; 4] =
    const_keccak_sel(b"getAllTransactionFilterers()");
const SEL_ARB_OWNER_SETFILTEREDFUNDSRECIPIENT: [u8; 4] =
    const_keccak_sel(b"setFilteredFundsRecipient(address)");
const SEL_ARB_OWNER_GETFILTEREDFUNDSRECIPIENT: [u8; 4] =
    const_keccak_sel(b"getFilteredFundsRecipient()");
const SEL_ARB_OWNER_SETL1BASEFEEESTIMATEINERTIA: [u8; 4] =
    const_keccak_sel(b"setL1BaseFeeEstimateInertia(uint64)");
const SEL_ARB_OWNER_SETL2BASEFEE: [u8; 4] = const_keccak_sel(b"setL2BaseFee(uint256)");
const SEL_ARB_OWNER_SETMINIMUML2BASEFEE: [u8; 4] =
    const_keccak_sel(b"setMinimumL2BaseFee(uint256)");
const SEL_ARB_OWNER_SETSPEEDLIMIT: [u8; 4] = const_keccak_sel(b"setSpeedLimit(uint64)");
const SEL_ARB_OWNER_SETMAXTXGASLIMIT: [u8; 4] = const_keccak_sel(b"setMaxTxGasLimit(uint64)");
const SEL_ARB_OWNER_SETMAXBLOCKGASLIMIT: [u8; 4] = const_keccak_sel(b"setMaxBlockGasLimit(uint64)");
const SEL_ARB_OWNER_SETL2GASPRICINGINERTIA: [u8; 4] =
    const_keccak_sel(b"setL2GasPricingInertia(uint64)");
const SEL_ARB_OWNER_SETL2GASBACKLOGTOLERANCE: [u8; 4] =
    const_keccak_sel(b"setL2GasBacklogTolerance(uint64)");
const SEL_ARB_OWNER_GETNETWORKFEEACCOUNT: [u8; 4] = const_keccak_sel(b"getNetworkFeeAccount()");
const SEL_ARB_OWNER_GETINFRAFEEACCOUNT: [u8; 4] = const_keccak_sel(b"getInfraFeeAccount()");
const SEL_ARB_OWNER_SETNETWORKFEEACCOUNT: [u8; 4] =
    const_keccak_sel(b"setNetworkFeeAccount(address)");
const SEL_ARB_OWNER_SETINFRAFEEACCOUNT: [u8; 4] = const_keccak_sel(b"setInfraFeeAccount(address)");
const SEL_ARB_OWNER_SCHEDULEARBOSUPGRADE: [u8; 4] =
    const_keccak_sel(b"scheduleArbOSUpgrade(uint64,uint64)");
const SEL_ARB_OWNER_SETL1PRICINGEQUILIBRATIONUNITS: [u8; 4] =
    const_keccak_sel(b"setL1PricingEquilibrationUnits(uint256)");
const SEL_ARB_OWNER_SETL1PRICINGINERTIA: [u8; 4] = const_keccak_sel(b"setL1PricingInertia(uint64)");
const SEL_ARB_OWNER_SETL1PRICINGREWARDRECIPIENT: [u8; 4] =
    const_keccak_sel(b"setL1PricingRewardRecipient(address)");
const SEL_ARB_OWNER_SETL1PRICINGREWARDRATE: [u8; 4] =
    const_keccak_sel(b"setL1PricingRewardRate(uint64)");
const SEL_ARB_OWNER_SETL1PRICEPERUNIT: [u8; 4] = const_keccak_sel(b"setL1PricePerUnit(uint256)");
const SEL_ARB_OWNER_SETPARENTGASFLOORPERTOKEN: [u8; 4] =
    const_keccak_sel(b"setParentGasFloorPerToken(uint64)");
const SEL_ARB_OWNER_SETPERBATCHGASCHARGE: [u8; 4] =
    const_keccak_sel(b"setPerBatchGasCharge(uint64)");
const SEL_ARB_OWNER_SETBROTLICOMPRESSIONLEVEL: [u8; 4] =
    const_keccak_sel(b"setBrotliCompressionLevel(uint64)");
const SEL_ARB_OWNER_SETAMORTIZEDCOSTCAPBIPS: [u8; 4] =
    const_keccak_sel(b"setAmortizedCostCapBips(uint64)");
const SEL_ARB_OWNER_RELEASEL1PRICERSURPLUSFUNDS: [u8; 4] =
    const_keccak_sel(b"releaseL1PricerSurplusFunds(uint64)");
const SEL_ARB_OWNER_SETINKPRICE: [u8; 4] = const_keccak_sel(b"setInkPrice(uint32)");
const SEL_ARB_OWNER_SETWASMMAXSTACKDEPTH: [u8; 4] =
    const_keccak_sel(b"setWasmMaxStackDepth(uint32)");
const SEL_ARB_OWNER_SETWASMFREEPAGES: [u8; 4] = const_keccak_sel(b"setWasmFreePages(uint16)");
const SEL_ARB_OWNER_SETWASMPAGEGAS: [u8; 4] = const_keccak_sel(b"setWasmPageGas(uint16)");
const SEL_ARB_OWNER_SETWASMPAGELIMIT: [u8; 4] = const_keccak_sel(b"setWasmPageLimit(uint32)");
const SEL_ARB_OWNER_SETWASMMAXSIZE: [u8; 4] = const_keccak_sel(b"setWasmMaxSize(uint32)");
const SEL_ARB_OWNER_SETWASMMININITGAS: [u8; 4] =
    const_keccak_sel(b"setWasmMinInitGas(uint8,uint16)");
const SEL_ARB_OWNER_SETWASMINITCOSTSCALAR: [u8; 4] =
    const_keccak_sel(b"setWasmInitCostScalar(uint64)");
const SEL_ARB_OWNER_SETWASMEXPIRYDAYS: [u8; 4] = const_keccak_sel(b"setWasmExpiryDays(uint16)");
const SEL_ARB_OWNER_SETWASMKEEPALIVEDAYS: [u8; 4] =
    const_keccak_sel(b"setWasmKeepaliveDays(uint16)");
const SEL_ARB_OWNER_SETWASMBLOCKCACHESIZE: [u8; 4] =
    const_keccak_sel(b"setWasmBlockCacheSize(uint16)");
const SEL_ARB_OWNER_ADDWASMCACHEMANAGER: [u8; 4] =
    const_keccak_sel(b"addWasmCacheManager(address)");
const SEL_ARB_OWNER_REMOVEWASMCACHEMANAGER: [u8; 4] =
    const_keccak_sel(b"removeWasmCacheManager(address)");
const SEL_ARB_OWNER_SETCHAINCONFIG: [u8; 4] = const_keccak_sel(b"setChainConfig(string)");
const SEL_ARB_OWNER_SETCALLDATAPRICEINCREASE: [u8; 4] =
    const_keccak_sel(b"setCalldataPriceIncrease(bool)");
const SEL_ARB_OWNER_SETGASBACKLOG: [u8; 4] = const_keccak_sel(b"setGasBacklog(uint64)");
const SEL_ARB_OWNER_SETGASPRICINGCONSTRAINTS: [u8; 4] =
    const_keccak_sel(b"setGasPricingConstraints(uint64[3][])");
const SEL_ARB_OWNER_SETMULTIGASPRICINGCONSTRAINTS: [u8; 4] =
    const_keccak_sel(b"setMultiGasPricingConstraints(((uint8,uint64)[],uint32,uint64,uint64)[])");
const SEL_ARB_OWNER_SETCOLLECTTIPS: [u8; 4] = const_keccak_sel(b"setCollectTips(bool)");
const SEL_ARB_OWNER_SETMAXSTYLUSCONTRACTFRAGMENTS: [u8; 4] =
    const_keccak_sel(b"setMaxStylusContractFragments(uint8)");
const SEL_ARB_OWNER_SETWASMACTIVATIONGAS: [u8; 4] =
    const_keccak_sel(b"setWasmActivationGas(uint64)");

const SEL_ARB_WASM_ACTIVATEPROGRAM: [u8; 4] = const_keccak_sel(b"activateProgram(address)");
const SEL_ARB_WASM_STYLUSVERSION: [u8; 4] = const_keccak_sel(b"stylusVersion()");
const SEL_ARB_WASM_CODEHASHVERSION: [u8; 4] = const_keccak_sel(b"codehashVersion(bytes32)");
const SEL_ARB_WASM_CODEHASHKEEPALIVE: [u8; 4] = const_keccak_sel(b"codehashKeepalive(bytes32)");
const SEL_ARB_WASM_CODEHASHASMSIZE: [u8; 4] = const_keccak_sel(b"codehashAsmSize(bytes32)");
const SEL_ARB_WASM_PROGRAMVERSION: [u8; 4] = const_keccak_sel(b"programVersion(address)");
const SEL_ARB_WASM_PROGRAMINITGAS: [u8; 4] = const_keccak_sel(b"programInitGas(address)");
const SEL_ARB_WASM_PROGRAMMEMORYFOOTPRINT: [u8; 4] =
    const_keccak_sel(b"programMemoryFootprint(address)");
const SEL_ARB_WASM_PROGRAMTIMELEFT: [u8; 4] = const_keccak_sel(b"programTimeLeft(address)");
const SEL_ARB_WASM_INKPRICE: [u8; 4] = const_keccak_sel(b"inkPrice()");
const SEL_ARB_WASM_MAXSTACKDEPTH: [u8; 4] = const_keccak_sel(b"maxStackDepth()");
const SEL_ARB_WASM_FREEPAGES: [u8; 4] = const_keccak_sel(b"freePages()");
const SEL_ARB_WASM_PAGEGAS: [u8; 4] = const_keccak_sel(b"pageGas()");
const SEL_ARB_WASM_PAGERAMP: [u8; 4] = const_keccak_sel(b"pageRamp()");
const SEL_ARB_WASM_PAGELIMIT: [u8; 4] = const_keccak_sel(b"pageLimit()");
const SEL_ARB_WASM_MININITGAS: [u8; 4] = const_keccak_sel(b"minInitGas()");
const SEL_ARB_WASM_INITCOSTSCALAR: [u8; 4] = const_keccak_sel(b"initCostScalar()");
const SEL_ARB_WASM_EXPIRYDAYS: [u8; 4] = const_keccak_sel(b"expiryDays()");
const SEL_ARB_WASM_KEEPALIVEDAYS: [u8; 4] = const_keccak_sel(b"keepaliveDays()");
const SEL_ARB_WASM_BLOCKCACHESIZE: [u8; 4] = const_keccak_sel(b"blockCacheSize()");
const SEL_ARB_WASM_ACTIVATIONGAS: [u8; 4] = const_keccak_sel(b"activationGas()");

const SEL_ARB_WASM_CACHE_ISCACHEMANAGER: [u8; 4] = const_keccak_sel(b"isCacheManager(address)");
const SEL_ARB_WASM_CACHE_ALLCACHEMANAGERS: [u8; 4] = const_keccak_sel(b"allCacheManagers()");
const SEL_ARB_WASM_CACHE_CACHECODEHASH: [u8; 4] = const_keccak_sel(b"cacheCodehash(bytes32)");
const SEL_ARB_WASM_CACHE_CACHEPROGRAM: [u8; 4] = const_keccak_sel(b"cacheProgram(address)");
const SEL_ARB_WASM_CACHE_EVICTCODEHASH: [u8; 4] = const_keccak_sel(b"evictCodehash(bytes32)");
const SEL_ARB_WASM_CACHE_CODEHASHISCACHED: [u8; 4] = const_keccak_sel(b"codehashIsCached(bytes32)");

const SEL_ARB_NATIVE_TOKEN_MANAGER_MINTNATIVETOKEN: [u8; 4] =
    const_keccak_sel(b"mintNativeToken(uint256)");
const SEL_ARB_NATIVE_TOKEN_MANAGER_BURNNATIVETOKEN: [u8; 4] =
    const_keccak_sel(b"burnNativeToken(uint256)");

const SEL_ARB_DEBUG_BECOMECHAINOWNER: [u8; 4] = const_keccak_sel(b"becomeChainOwner()");
const SEL_ARB_DEBUG_OVERWRITECONTRACTCODE: [u8; 4] =
    const_keccak_sel(b"overwriteContractCode(address,bytes)");
const SEL_ARB_DEBUG_EVENTS: [u8; 4] = const_keccak_sel(b"events(bool,bytes32)");
const SEL_ARB_DEBUG_EVENTSVIEW: [u8; 4] = const_keccak_sel(b"eventsView()");
const SEL_ARB_DEBUG_CUSTOMREVERT: [u8; 4] = const_keccak_sel(b"customRevert(uint64)");
const SEL_ARB_DEBUG_PANIC: [u8; 4] = const_keccak_sel(b"panic()");
const SEL_ARB_DEBUG_LEGACYERROR: [u8; 4] = const_keccak_sel(b"legacyError()");

fn make_fn_arb_sys_arb_block_number() -> [u8; 4] {
    concat_arrays!(SEL_ARB_SYS_ARBBLOCKNUMBER)
}

fn make_fn_arb_sys_arb_block_hash(arbBlockNum: U) -> [u8; 36] {
    concat_arrays!(SEL_ARB_SYS_ARBBLOCKHASH, arbBlockNum.0)
}

fn make_fn_arb_sys_arb_chain_i_d() -> [u8; 4] {
    concat_arrays!(SEL_ARB_SYS_ARBCHAINID)
}

fn make_fn_arb_sys_arb_o_s_version() -> [u8; 4] {
    concat_arrays!(SEL_ARB_SYS_ARBOSVERSION)
}

fn make_fn_arb_sys_get_storage_gas_available() -> [u8; 4] {
    concat_arrays!(SEL_ARB_SYS_GETSTORAGEGASAVAILABLE)
}

fn make_fn_arb_sys_is_top_level_call() -> [u8; 4] {
    concat_arrays!(SEL_ARB_SYS_ISTOPLEVELCALL)
}

fn make_fn_arb_sys_map_l1_sender_contract_address_to_l2_alias(
    sender: [u8; 20],
    unused: [u8; 20],
) -> [u8; 68] {
    concat_arrays!(
...
fn make_fn_arb_sys_was_my_callers_address_aliased() -> [u8; 4] {
    concat_arrays!(SEL_ARB_SYS_WASMYCALLERSADDRESSALIASED)
}

fn make_fn_arb_sys_my_callers_address_without_aliasing() -> [u8; 4] {
    concat_arrays!(SEL_ARB_SYS_MYCALLERSADDRESSWITHOUTALIASING)
}

fn make_fn_arb_sys_withdraw_eth(destination: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_SYS_WITHDRAWETH, leftpad_addr(destination))
}
...
fn make_fn_arb_sys_send_merkle_tree_state() -> [u8; 4] {
    concat_arrays!(SEL_ARB_SYS_SENDMERKLETREESTATE)
}

fn make_fn_arb_info_get_balance(account: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_INFO_GETBALANCE, leftpad_addr(account))
}

fn make_fn_arb_info_get_code(account: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_INFO_GETCODE, leftpad_addr(account))
}

fn make_fn_arb_address_table_address_exists(addr: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_ADDRESS_TABLE_ADDRESSEXISTS, leftpad_addr(addr))
}

fn make_fn_arb_address_table_compress(addr: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_ADDRESS_TABLE_COMPRESS, leftpad_addr(addr))
}
...
fn make_fn_arb_address_table_lookup(addr: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_ADDRESS_TABLE_LOOKUP, leftpad_addr(addr))
}

fn make_fn_arb_address_table_lookup_index(index: U) -> [u8; 36] {
    concat_arrays!(SEL_ARB_ADDRESS_TABLE_LOOKUPINDEX, index.0)
}

fn make_fn_arb_address_table_register(addr: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_ADDRESS_TABLE_REGISTER, leftpad_addr(addr))
}

fn make_fn_arb_address_table_size() -> [u8; 4] {
    concat_arrays!(SEL_ARB_ADDRESS_TABLE_SIZE)
}
...
fn make_fn_arb_function_table_size(addr: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_FUNCTION_TABLE_SIZE, leftpad_addr(addr))
}

fn make_fn_arb_function_table_get(addr: [u8; 20], index: U) -> [u8; 68] {
    concat_arrays!(SEL_ARB_FUNCTION_TABLE_GET, leftpad_addr(addr), index.0)
}

fn make_fn_arbos_test_burn_arb_gas(gasAmount: U) -> [u8; 36] {
    concat_arrays!(SEL_ARBOS_TEST_BURNARBGAS, gasAmount.0)
}

fn make_fn_arb_owner_public_is_chain_owner(addr: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_ISCHAINOWNER, leftpad_addr(addr))
}
...
fn make_fn_arb_owner_public_get_all_chain_owners() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_GETALLCHAINOWNERS)
}

fn make_fn_arb_owner_public_get_native_token_management_from() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_GETNATIVETOKENMANAGEMENTFROM)
}

fn make_fn_arb_owner_public_is_native_token_owner(addr: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_ISNATIVETOKENOWNER, leftpad_addr(addr))
}

fn make_fn_arb_owner_public_get_all_native_token_owners() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_GETALLNATIVETOKENOWNERS)
}

fn make_fn_arb_owner_public_get_transaction_filtering_from() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_GETTRANSACTIONFILTERINGFROM)
}
...
fn make_fn_arb_owner_public_get_all_transaction_filterers() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_GETALLTRANSACTIONFILTERERS)
}

fn make_fn_arb_owner_public_get_filtered_funds_recipient() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_GETFILTEREDFUNDSRECIPIENT)
}

fn make_fn_arb_owner_public_get_network_fee_account() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_GETNETWORKFEEACCOUNT)
}

fn make_fn_arb_owner_public_get_infra_fee_account() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_GETINFRAFEEACCOUNT)
}

fn make_fn_arb_owner_public_get_brotli_compression_level() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_GETBROTLICOMPRESSIONLEVEL)
}

fn make_fn_arb_owner_public_get_parent_gas_floor_per_token() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_GETPARENTGASFLOORPERTOKEN)
}

fn make_fn_arb_owner_public_get_scheduled_upgrade() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_GETSCHEDULEDUPGRADE)
}

fn make_fn_arb_owner_public_is_calldata_price_increase_enabled() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_ISCALLDATAPRICEINCREASEENABLED)
}

fn make_fn_arb_owner_public_get_collect_tips() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_GETCOLLECTTIPS)
}

fn make_fn_arb_owner_public_get_max_stylus_contract_fragments() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_PUBLIC_GETMAXSTYLUSCONTRACTFRAGMENTS)
}
...
fn make_fn_arb_gas_info_get_prices_in_wei() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETPRICESINWEI)
}
...
fn make_fn_arb_gas_info_get_prices_in_arb_gas() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETPRICESINARBGAS)
}

fn make_fn_arb_gas_info_get_gas_accounting_params() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETGASACCOUNTINGPARAMS)
}

fn make_fn_arb_gas_info_get_max_tx_gas_limit() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETMAXTXGASLIMIT)
}

fn make_fn_arb_gas_info_get_minimum_gas_price() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETMINIMUMGASPRICE)
}

fn make_fn_arb_gas_info_get_l1_base_fee_estimate() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETL1BASEFEEESTIMATE)
}

fn make_fn_arb_gas_info_get_l1_base_fee_estimate_inertia() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETL1BASEFEEESTIMATEINERTIA)
}

fn make_fn_arb_gas_info_get_l1_reward_rate() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETL1REWARDRATE)
}

fn make_fn_arb_gas_info_get_l1_reward_recipient() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETL1REWARDRECIPIENT)
}

fn make_fn_arb_gas_info_get_l1_gas_price_estimate() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETL1GASPRICEESTIMATE)
}

fn make_fn_arb_gas_info_get_current_tx_l1_gas_fees() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETCURRENTTXL1GASFEES)
}

fn make_fn_arb_gas_info_get_gas_backlog() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETGASBACKLOG)
}

fn make_fn_arb_gas_info_get_pricing_inertia() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETPRICINGINERTIA)
}

fn make_fn_arb_gas_info_get_gas_backlog_tolerance() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETGASBACKLOGTOLERANCE)
}

fn make_fn_arb_gas_info_get_l1_pricing_surplus() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETL1PRICINGSURPLUS)
}

fn make_fn_arb_gas_info_get_per_batch_gas_charge() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETPERBATCHGASCHARGE)
}

fn make_fn_arb_gas_info_get_amortized_cost_cap_bips() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETAMORTIZEDCOSTCAPBIPS)
}

fn make_fn_arb_gas_info_get_l1_fees_available() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETL1FEESAVAILABLE)
}

fn make_fn_arb_gas_info_get_l1_pricing_equilibration_units() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETL1PRICINGEQUILIBRATIONUNITS)
}

fn make_fn_arb_gas_info_get_last_l1_pricing_update_time() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETLASTL1PRICINGUPDATETIME)
}

fn make_fn_arb_gas_info_get_l1_pricing_funds_due_for_rewards() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETL1PRICINGFUNDSDUEFORREWARDS)
}

fn make_fn_arb_gas_info_get_l1_pricing_units_since_update() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETL1PRICINGUNITSSINCEUPDATE)
}

fn make_fn_arb_gas_info_get_last_l1_pricing_surplus() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETLASTL1PRICINGSURPLUS)
}

fn make_fn_arb_gas_info_get_max_block_gas_limit() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETMAXBLOCKGASLIMIT)
}

fn make_fn_arb_gas_info_get_gas_pricing_constraints() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETGASPRICINGCONSTRAINTS)
}

fn make_fn_arb_gas_info_get_multi_gas_pricing_constraints() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETMULTIGASPRICINGCONSTRAINTS)
}

fn make_fn_arb_gas_info_get_multi_gas_base_fee() -> [u8; 4] {
    concat_arrays!(SEL_ARB_GAS_INFO_GETMULTIGASBASEFEE)
}
...
fn make_fn_arb_aggregator_get_default_aggregator() -> [u8; 4] {
    concat_arrays!(SEL_ARB_AGGREGATOR_GETDEFAULTAGGREGATOR)
}

fn make_fn_arb_aggregator_get_batch_posters() -> [u8; 4] {
    concat_arrays!(SEL_ARB_AGGREGATOR_GETBATCHPOSTERS)
}
...
fn make_fn_arb_aggregator_set_fee_collector(
    batchPoster: [u8; 20],
    newFeeCollector: [u8; 20],
) -> [u8; 68] {
    concat_arrays!(
...
fn make_fn_arb_aggregator_get_tx_base_fee(aggregator: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_AGGREGATOR_GETTXBASEFEE, leftpad_addr(aggregator))
}
...
fn make_fn_arb_retryable_tx_redeem(ticketId: [u8; 32]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_RETRYABLE_TX_REDEEM, ticketId)
}

fn make_fn_arb_retryable_tx_get_lifetime() -> [u8; 4] {
    concat_arrays!(SEL_ARB_RETRYABLE_TX_GETLIFETIME)
}

fn make_fn_arb_retryable_tx_get_timeout(ticketId: [u8; 32]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_RETRYABLE_TX_GETTIMEOUT, ticketId)
}

fn make_fn_arb_retryable_tx_keepalive(ticketId: [u8; 32]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_RETRYABLE_TX_KEEPALIVE, ticketId)
}

fn make_fn_arb_retryable_tx_get_beneficiary(ticketId: [u8; 32]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_RETRYABLE_TX_GETBENEFICIARY, ticketId)
}

fn make_fn_arb_retryable_tx_cancel(ticketId: [u8; 32]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_RETRYABLE_TX_CANCEL, ticketId)
}

fn make_fn_arb_retryable_tx_get_current_redeemer() -> [u8; 4] {
    concat_arrays!(SEL_ARB_RETRYABLE_TX_GETCURRENTREDEEMER)
}

fn make_fn_arb_retryable_tx_submit_retryable<
    const N: usize,
    const PAD: usize,
    const TOTAL: usize,
>(
...
fn make_fn_arb_statistics_get_stats() -> [u8; 4] {
    concat_arrays!(SEL_ARB_STATISTICS_GETSTATS)
}

fn make_fn_arb_owner_add_chain_owner(newOwner: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_ADDCHAINOWNER, leftpad_addr(newOwner))
}

fn make_fn_arb_owner_remove_chain_owner(owner: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_REMOVECHAINOWNER, leftpad_addr(owner))
}

fn make_fn_arb_owner_is_chain_owner(addr: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_ISCHAINOWNER, leftpad_addr(addr))
}

fn make_fn_arb_owner_get_all_chain_owners() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_GETALLCHAINOWNERS)
}
...
fn make_fn_arb_owner_add_native_token_owner(newOwner: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_ADDNATIVETOKENOWNER, leftpad_addr(newOwner))
}

fn make_fn_arb_owner_remove_native_token_owner(owner: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_REMOVENATIVETOKENOWNER, leftpad_addr(owner))
}

fn make_fn_arb_owner_is_native_token_owner(addr: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_ISNATIVETOKENOWNER, leftpad_addr(addr))
}

fn make_fn_arb_owner_get_all_native_token_owners() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_GETALLNATIVETOKENOWNERS)
}
...
fn make_fn_arb_owner_is_transaction_filterer(addr: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_ISTRANSACTIONFILTERER, leftpad_addr(addr))
}

fn make_fn_arb_owner_get_all_transaction_filterers() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_GETALLTRANSACTIONFILTERERS)
}
...
fn make_fn_arb_owner_get_filtered_funds_recipient() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_GETFILTEREDFUNDSRECIPIENT)
}
...
fn make_fn_arb_owner_set_l2_base_fee(priceInWei: U) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETL2BASEFEE, priceInWei.0)
}

fn make_fn_arb_owner_set_minimum_l2_base_fee(priceInWei: U) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETMINIMUML2BASEFEE, priceInWei.0)
}

fn make_fn_arb_owner_set_speed_limit(speedLimit: u64) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETSPEEDLIMIT, leftpad_u64(speedLimit))
}

fn make_fn_arb_owner_set_max_tx_gas_limit(maxTxGasLimit: u64) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETMAXTXGASLIMIT, leftpad_u64(maxTxGasLimit))
}
...
fn make_fn_arb_owner_set_l2_gas_pricing_inertia(inertia: u64) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETL2GASPRICINGINERTIA, leftpad_u64(inertia))
}
...
fn make_fn_arb_owner_get_network_fee_account() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_GETNETWORKFEEACCOUNT)
}

fn make_fn_arb_owner_get_infra_fee_account() -> [u8; 4] {
    concat_arrays!(SEL_ARB_OWNER_GETINFRAFEEACCOUNT)
}
...
fn make_fn_arb_owner_set_l1_pricing_equilibration_units(units: U) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETL1PRICINGEQUILIBRATIONUNITS, units.0)
}

fn make_fn_arb_owner_set_l1_pricing_inertia(inertia: u64) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETL1PRICINGINERTIA, leftpad_u64(inertia))
}
...
fn make_fn_arb_owner_set_l1_price_per_unit(pricePerUnit: U) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETL1PRICEPERUNIT, pricePerUnit.0)
}
...
fn make_fn_arb_owner_set_per_batch_gas_charge(charge: u64) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETPERBATCHGASCHARGE, leftpad_u64(charge))
}

fn make_fn_arb_owner_set_brotli_compression_level(level: u64) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETBROTLICOMPRESSIONLEVEL, leftpad_u64(level))
}

fn make_fn_arb_owner_set_amortized_cost_cap_bips(capBips: u64) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETAMORTIZEDCOSTCAPBIPS, leftpad_u64(capBips))
}
...
fn make_fn_arb_owner_set_ink_price(price: u32) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETINKPRICE, leftpad_u32(price))
}

fn make_fn_arb_owner_set_wasm_max_stack_depth(depth: u32) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETWASMMAXSTACKDEPTH, leftpad_u32(depth))
}

fn make_fn_arb_owner_set_wasm_free_pages(amount: u16) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETWASMFREEPAGES, leftpad_u16(amount))
}

fn make_fn_arb_owner_set_wasm_page_gas(amount: u16) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETWASMPAGEGAS, leftpad_u16(amount))
}

fn make_fn_arb_owner_set_wasm_page_limit(limit: u32) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETWASMPAGELIMIT, leftpad_u32(limit))
}

fn make_fn_arb_owner_set_wasm_max_size(size: u32) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETWASMMAXSIZE, leftpad_u32(size))
}
...
fn make_fn_arb_owner_set_wasm_init_cost_scalar(percent: u64) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETWASMINITCOSTSCALAR, leftpad_u64(percent))
}

fn make_fn_arb_owner_set_wasm_expiry_days(days: u16) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETWASMEXPIRYDAYS, leftpad_u16(days))
}

fn make_fn_arb_owner_set_wasm_keepalive_days(days: u16) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETWASMKEEPALIVEDAYS, leftpad_u16(days))
}

fn make_fn_arb_owner_set_wasm_block_cache_size(size: u16) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETWASMBLOCKCACHESIZE, leftpad_u16(size))
}

fn make_fn_arb_owner_add_wasm_cache_manager(manager: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_ADDWASMCACHEMANAGER, leftpad_addr(manager))
}

fn make_fn_arb_owner_remove_wasm_cache_manager(manager: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_REMOVEWASMCACHEMANAGER, leftpad_addr(manager))
}
...
fn make_fn_arb_owner_set_calldata_price_increase(enable: bool) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETCALLDATAPRICEINCREASE, leftpad_bool(enable))
}

fn make_fn_arb_owner_set_gas_backlog(backlog: u64) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETGASBACKLOG, leftpad_u64(backlog))
}

fn make_fn_arb_owner_set_gas_pricing_constraints<const N: usize, const TOTAL: usize>(
    tail: &[u8; N],
) -> [u8; TOTAL] {
...
fn make_fn_arb_owner_set_multi_gas_pricing_constraints<const N: usize, const TOTAL: usize>(
    tail: &[u8; N],
) -> [u8; TOTAL] {
    assert_eq!(TOTAL, 4 + N, "total cap not consistent");
...
fn make_fn_arb_owner_set_collect_tips(collectTips: bool) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETCOLLECTTIPS, leftpad_bool(collectTips))
}
...
fn make_fn_arb_owner_set_wasm_activation_gas(gas: u64) -> [u8; 36] {
    concat_arrays!(SEL_ARB_OWNER_SETWASMACTIVATIONGAS, leftpad_u64(gas))
}

fn make_fn_arb_wasm_activate_program(program: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_WASM_ACTIVATEPROGRAM, leftpad_addr(program))
}

fn make_fn_arb_wasm_stylus_version() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_STYLUSVERSION)
}

fn make_fn_arb_wasm_codehash_version(codehash: [u8; 32]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_WASM_CODEHASHVERSION, codehash)
}

fn make_fn_arb_wasm_codehash_keepalive(codehash: [u8; 32]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_WASM_CODEHASHKEEPALIVE, codehash)
}

fn make_fn_arb_wasm_codehash_asm_size(codehash: [u8; 32]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_WASM_CODEHASHASMSIZE, codehash)
}

fn make_fn_arb_wasm_program_version(program: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_WASM_PROGRAMVERSION, leftpad_addr(program))
}

fn make_fn_arb_wasm_program_init_gas(program: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_WASM_PROGRAMINITGAS, leftpad_addr(program))
}

fn make_fn_arb_wasm_program_memory_footprint(program: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_WASM_PROGRAMMEMORYFOOTPRINT, leftpad_addr(program))
}

fn make_fn_arb_wasm_program_time_left(program: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_WASM_PROGRAMTIMELEFT, leftpad_addr(program))
}

fn make_fn_arb_wasm_ink_price() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_INKPRICE)
}

fn make_fn_arb_wasm_max_stack_depth() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_MAXSTACKDEPTH)
}

fn make_fn_arb_wasm_free_pages() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_FREEPAGES)
}

fn make_fn_arb_wasm_page_gas() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_PAGEGAS)
}

fn make_fn_arb_wasm_page_ramp() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_PAGERAMP)
}

fn make_fn_arb_wasm_page_limit() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_PAGELIMIT)
}

fn make_fn_arb_wasm_min_init_gas() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_MININITGAS)
}

fn make_fn_arb_wasm_init_cost_scalar() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_INITCOSTSCALAR)
}

fn make_fn_arb_wasm_expiry_days() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_EXPIRYDAYS)
}

fn make_fn_arb_wasm_keepalive_days() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_KEEPALIVEDAYS)
}

fn make_fn_arb_wasm_block_cache_size() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_BLOCKCACHESIZE)
}

fn make_fn_arb_wasm_activation_gas() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_ACTIVATIONGAS)
}

fn make_fn_arb_wasm_cache_is_cache_manager(manager: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_WASM_CACHE_ISCACHEMANAGER, leftpad_addr(manager))
}

fn make_fn_arb_wasm_cache_all_cache_managers() -> [u8; 4] {
    concat_arrays!(SEL_ARB_WASM_CACHE_ALLCACHEMANAGERS)
}

fn make_fn_arb_wasm_cache_cache_codehash(codehash: [u8; 32]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_WASM_CACHE_CACHECODEHASH, codehash)
}

fn make_fn_arb_wasm_cache_cache_program(addr: [u8; 20]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_WASM_CACHE_CACHEPROGRAM, leftpad_addr(addr))
}

fn make_fn_arb_wasm_cache_evict_codehash(codehash: [u8; 32]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_WASM_CACHE_EVICTCODEHASH, codehash)
}

fn make_fn_arb_wasm_cache_codehash_is_cached(codehash: [u8; 32]) -> [u8; 36] {
    concat_arrays!(SEL_ARB_WASM_CACHE_CODEHASHISCACHED, codehash)
}

fn make_fn_arb_native_token_manager_mint_native_token(amount: U) -> [u8; 36] {
    concat_arrays!(SEL_ARB_NATIVE_TOKEN_MANAGER_MINTNATIVETOKEN, amount.0)
}

fn make_fn_arb_native_token_manager_burn_native_token(amount: U) -> [u8; 36] {
    concat_arrays!(SEL_ARB_NATIVE_TOKEN_MANAGER_BURNNATIVETOKEN, amount.0)
}

fn make_fn_arb_debug_become_chain_owner() -> [u8; 4] {
    concat_arrays!(SEL_ARB_DEBUG_BECOMECHAINOWNER)
}

fn make_fn_arb_debug_overwrite_contract_code<
    const N: usize,
    const PAD: usize,
    const TOTAL: usize,
>(
...
fn make_fn_arb_debug_events(flag: bool, value: [u8; 32]) -> [u8; 68] {
    concat_arrays!(SEL_ARB_DEBUG_EVENTS, leftpad_bool(flag), value)
}

fn make_fn_arb_debug_events_view() -> [u8; 4] {
    concat_arrays!(SEL_ARB_DEBUG_EVENTSVIEW)
}

fn make_fn_arb_debug_custom_revert(number: u64) -> [u8; 36] {
    concat_arrays!(SEL_ARB_DEBUG_CUSTOMREVERT, leftpad_u64(number))
}

fn make_fn_arb_debug_panic() -> [u8; 4] {
    concat_arrays!(SEL_ARB_DEBUG_PANIC)
}

fn make_fn_arb_debug_legacy_error() -> [u8; 4] {
    concat_arrays!(SEL_ARB_DEBUG_LEGACYERROR)
}
...
mod tests {
    ...
    fn test_arb_sys_send_tx_to_l1_dynamic() {
        let destination = [0x11u8; 20];
        {
            ...
            let exp = p_arb_sys::sendTxToL1Call {
                destination: AAddress::from(destination),
    ...
    fn test_arb_address_table_decompress_dynamic() {
        let offset = U::from([0x22u8; 32]);
        {
            ...
            let exp = p_arb_address_table::decompressCall {
                buf: Bytes::from(buf.to_vec()),
                offset: AU::from_be_bytes(offset.0),
            }
            .abi_encode();
            assert_eq!(
                exp,
    ...
    fn test_arb_function_table_upload_dynamic() {
        {
            ...
            let exp = p_arb_function_table::uploadCall {
                buf: Bytes::from(buf.to_vec()),
            }
            .abi_encode();
            assert_eq!(
    ...
    fn test_arb_retryable_tx_submit_retryable_dynamic() {
        ...
        let retryTo = [0x11u8; 20];
        {
            // size 0
            let buf = [0xabu8; 0];
            let v = make_fn_arb_retryable_tx_submit_retryable::<0, 0, 388>(
                requestId,
                l1BaseFee,
                deposit,
                callvalue,
            ...
            .to_vec();
            let exp = p_arb_retryable_tx::submitRetryableCall {
                requestId: FixedBytes::from(requestId),
                l1BaseFee: AU::from_be_bytes(l1BaseFee.0),
                deposit: AU::from_be_bytes(deposit.0),
                callvalue: AU::from_be_bytes(callvalue.0),
                gasFeeCap: AU::from_be_bytes(gasFeeCap.0),
                gasLimit: gasLimit,
                maxSubmissionFee: AU::from_be_bytes(maxSubmissionFee.0),
                feeRefundAddress: AAddress::from(feeRefundAddress),
                beneficiary: AAddress::from(beneficiary),
                retryTo: AAddress::from(retryTo),
                retryData: Bytes::from(buf.to_vec()),
            }
            .abi_encode();
            assert_eq!(
                exp,
        ...
        }
        {
            // size 1
            let buf = [0xabu8; 1];
            let v = make_fn_arb_retryable_tx_submit_retryable::<1, 31, 420>(
                requestId,
                l1BaseFee,
                deposit,
                callvalue,
        ...
        }
        {
            // size 32
            let buf = [0xabu8; 32];
            let v = make_fn_arb_retryable_tx_submit_retryable::<32, 0, 420>(
                requestId,
                l1BaseFee,
                deposit,
                callvalue,
        ...
        }
        {
            // size 37
            let buf = [0xabu8; 37];
            let v = make_fn_arb_retryable_tx_submit_retryable::<37, 27, 452>(
                requestId,
                l1BaseFee,
                deposit,
                callvalue,
    ...
    fn test_arb_owner_set_chain_config_dynamic() {
        {
            ...
            let exp = p_arb_owner::setChainConfigCall {
                chainConfig: unsafe { String::from_utf8_unchecked(buf.to_vec()) },
            }
            .abi_encode();
            assert_eq!(
                exp,
    ...
    fn test_arb_owner_set_gas_pricing_constraints_tail() {
        ...
        assert_eq!(
            &exp[..4],
            &SEL_ARB_OWNER_SETGASPRICINGCONSTRAINTS[..],
            "selector mismatch"
        );
        {
            // size 0
    ...
    fn test_arb_owner_set_multi_gas_pricing_constraints_tail() {
        ...
        assert_eq!(
            &exp[..4],
            &SEL_ARB_OWNER_SETMULTIGASPRICINGCONSTRAINTS[..],
            "selector mismatch"
        );
        {
            // size 0
    ...
    fn test_arb_debug_overwrite_contract_code_dynamic() {
        let target = [0x11u8; 20];
        {
            ...
            let exp = p_arb_debug::overwriteContractCodeCall {
                target: AAddress::from(target),
                newCode: Bytes::from(buf.to_vec()),
            }
            .abi_encode();
            assert_eq!(
                exp,
...
// and therefore use the real `call` with `value: &U`.
// ============================================================================
...
fn usize_at(v: &[u8], off: usize) -> usize {
    let mut x = 0usize;
    for i in 0..32 {
        x = x.wrapping_mul(256).wrapping_add(v[off + i] as usize);
    }
    x
}
...
fn u64_from(w: U) -> u64 {
    let b = w.0;
    let a: [u8; 8] = b[24..32].try_into().unwrap();
    u64::from_be_bytes(a)
}
#[inline]
fn u32_from(w: U) -> u32 {
    let b = w.0;
    let a: [u8; 4] = b[28..32].try_into().unwrap();
    u32::from_be_bytes(a)
}
#[inline]
fn u16_from(w: U) -> u16 {
    let b = w.0;
    let a: [u8; 2] = b[30..32].try_into().unwrap();
    u16::from_be_bytes(a)
}
#[inline]
fn u8_from(w: U) -> u8 {
...
#[inline]
fn addr_from(w: U) -> [u8; 20] {
    let b = w.0;
    let a: [u8; 20] = b[12..32].try_into().unwrap();
    a
}

/// static_call returning a single ABI word.
#[inline]
fn static_word(addr: [u8; 20], cd: &[u8]) -> U {
    U::from(static_call_slice::<32>(addr, cd, u64::MAX, 0).2)
}
...
fn static_words<const N: usize, const CAP: usize>(addr: [u8; 20], cd: &[u8]) -> [U; N] {
    let (_, _, rd) = static_call_slice::<CAP>(addr, cd, u64::MAX, 0);
    let mut out = [U::ZERO; N];
    for i in 0..N {
        out[i] = word_at(&rd, i * 32);
    }
    out
}

/// call returning a single ABI word.
#[inline]
fn call_word(addr: [u8; 20], cd: &[u8], value: &U) -> U {
    U::from(call_slice::<32>(addr, cd, value, u64::MAX, 0).2)
}

/// call returning `n` consecutive fixed words. `CAP` must equal `32 * N`.
fn call_words<const N: usize, const CAP: usize>(addr: [u8; 20], cd: &[u8], value: &U) -> [U; N] {
    let (_, _, rd) = call_slice::<CAP>(addr, cd, value, u64::MAX, 0);
    let mut out = [U::ZERO; N];
    for i in 0..N {
        out[i] = word_at(&rd, i * 32);
    }
...
fn decode_u256_vec(v: &[u8]) -> Option<Vec<U>> {
    ...
    let mut out = Vec::with_capacity(n);
    for i in 0..n {
        out.push(word_at_vec(v, ao + 32 + 32 * i));
    }
    Some(out)
}
...
fn word_at_vec(v: &[u8], off: usize) -> U {
    let mut w = [0u8; 32];
    w.copy_from_slice(&v[off..off + 32]);
    U::from(w)
}
...
fn decode_multicons(v: &[u8]) -> Option<Vec<RawResourceConstraint>> {
    ...
    for i in 0..n {
        ...
        out.push(RawResourceConstraint {
            resources,
            adjustment_window_secs: adjustment,
...
pub fn call_arb_sys_arb_block_number() -> U {
    let cd = make_fn_arb_sys_arb_block_number();
    static_word(ADDR_ARB_SYS, &cd)
}

pub fn call_arb_sys_arb_block_hash(arbBlockNum: U) -> U {
    let cd = make_fn_arb_sys_arb_block_hash(arbBlockNum);
    static_word(ADDR_ARB_SYS, &cd)
}

pub fn call_arb_sys_arb_chain_i_d() -> U {
    let cd = make_fn_arb_sys_arb_chain_i_d();
    static_word(ADDR_ARB_SYS, &cd)
}

pub fn call_arb_sys_arb_o_s_version() -> U {
    let cd = make_fn_arb_sys_arb_o_s_version();
    static_word(ADDR_ARB_SYS, &cd)
}

pub fn call_arb_sys_get_storage_gas_available() -> U {
    let cd = make_fn_arb_sys_get_storage_gas_available();
    static_word(ADDR_ARB_SYS, &cd)
}

pub fn call_arb_sys_is_top_level_call() -> bool {
    let cd = make_fn_arb_sys_is_top_level_call();
    u8_from(static_word(ADDR_ARB_SYS, &cd)) != 0
}

pub fn call_arb_sys_map_l1_sender_contract_address_to_l2_alias(
    sender: [u8; 20],
    unused: [u8; 20],
) -> [u8; 20] {
    let cd = make_fn_arb_sys_map_l1_sender_contract_address_to_l2_alias(sender, unused);
    addr_from(static_word(ADDR_ARB_SYS, &cd))
}

pub fn call_arb_sys_was_my_callers_address_aliased() -> bool {
    let cd = make_fn_arb_sys_was_my_callers_address_aliased();
    u8_from(static_word(ADDR_ARB_SYS, &cd)) != 0
}

pub fn call_arb_sys_my_callers_address_without_aliasing() -> [u8; 20] {
    let cd = make_fn_arb_sys_my_callers_address_without_aliasing();
    addr_from(static_word(ADDR_ARB_SYS, &cd))
}

pub fn call_arb_sys_withdraw_eth(destination: [u8; 20], value: &U) -> U {
    let cd = make_fn_arb_sys_withdraw_eth(destination);
    call_word(ADDR_ARB_SYS, &cd, value)
}

pub fn call_arb_sys_send_tx_to_l1<const N: usize, const PAD: usize, const TOTAL: usize>(
    destination: [u8; 20],
    buf: &[u8; N],
    value: &U,
) -> U {
...
pub fn call_arb_sys_send_merkle_tree_state() -> Option<(U, [u8; 32], Vec<[u8; 32]>)> {
    ...
    let (rc, v) = static_call_vec(ADDR_ARB_SYS, &cd, u64::MAX, 0);
    if rc { decode_merkle(&v) } else { None }
}

pub fn call_arb_info_get_balance(account: [u8; 20]) -> U {
    let cd = make_fn_arb_info_get_balance(account);
    static_word(ADDR_ARB_INFO, &cd)
}

#[cfg(feature = "alloc")]
pub fn call_arb_info_get_code(account: [u8; 20]) -> Vec<u8> {
    let cd = make_fn_arb_info_get_code(account);
    let (_, v) = static_call_vec(ADDR_ARB_INFO, &cd, u64::MAX, 0);
    v
}

pub fn call_arb_address_table_address_exists(addr: [u8; 20]) -> bool {
    let cd = make_fn_arb_address_table_address_exists(addr);
    u8_from(static_word(ADDR_ARB_ADDRESS_TABLE, &cd)) != 0
}

#[cfg(feature = "alloc")]
pub fn call_arb_address_table_compress(addr: [u8; 20], value: &U) -> Vec<u8> {
    let cd = make_fn_arb_address_table_compress(addr);
    let (_, v) = call_vec(ADDR_ARB_ADDRESS_TABLE, &cd, value, u64::MAX, 0);
    v
}

pub fn call_arb_address_table_decompress<const N: usize, const PAD: usize, const TOTAL: usize>(
    offset: U,
    buf: &[u8; N],
) -> ([u8; 20], U) {
    let cd = make_fn_arb_address_table_decompress::<N, PAD, TOTAL>(offset, buf);
    let ws = static_words::<2, { 32 * 2 }>(ADDR_ARB_ADDRESS_TABLE, &cd);
    (addr_from(ws[0]), ws[1])
}

pub fn call_arb_address_table_lookup(addr: [u8; 20]) -> U {
    let cd = make_fn_arb_address_table_lookup(addr);
    static_word(ADDR_ARB_ADDRESS_TABLE, &cd)
}

pub fn call_arb_address_table_lookup_index(index: U) -> [u8; 20] {
    let cd = make_fn_arb_address_table_lookup_index(index);
    addr_from(static_word(ADDR_ARB_ADDRESS_TABLE, &cd))
}

pub fn call_arb_address_table_register(addr: [u8; 20], value: &U) -> U {
    let cd = make_fn_arb_address_table_register(addr);
    call_word(ADDR_ARB_ADDRESS_TABLE, &cd, value)
}

pub fn call_arb_address_table_size() -> U {
    let cd = make_fn_arb_address_table_size();
    static_word(ADDR_ARB_ADDRESS_TABLE, &cd)
}

pub fn call_arb_function_table_upload<const N: usize, const PAD: usize, const TOTAL: usize>(
    buf: &[u8; N],
    value: &U,
) -> bool {
    let cd = make_fn_arb_function_table_upload::<N, PAD, TOTAL>(buf);
    call_unit(ADDR_ARB_FUNCTION_TABLE, &cd, value, u64::MAX)
}

pub fn call_arb_function_table_size(addr: [u8; 20]) -> U {
    let cd = make_fn_arb_function_table_size(addr);
    static_word(ADDR_ARB_FUNCTION_TABLE, &cd)
}

pub fn call_arb_function_table_get(addr: [u8; 20], index: U) -> (U, bool, U) {
    let cd = make_fn_arb_function_table_get(addr, index);
    let ws = static_words::<3, { 32 * 3 }>(ADDR_ARB_FUNCTION_TABLE, &cd);
    (ws[0], u8_from(ws[1]) != 0, ws[2])
}

pub fn call_arbos_test_burn_arb_gas(gasAmount: U, value: &U) -> bool {
    let cd = make_fn_arbos_test_burn_arb_gas(gasAmount);
    call_unit(ADDR_ARBOS_TEST, &cd, value, u64::MAX)
}

pub fn call_arb_owner_public_is_chain_owner(addr: [u8; 20]) -> bool {
    let cd = make_fn_arb_owner_public_is_chain_owner(addr);
    u8_from(static_word(ADDR_ARB_OWNER_PUBLIC, &cd)) != 0
}

pub fn call_arb_owner_public_rectify_chain_owner(ownerToRectify: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_owner_public_rectify_chain_owner(ownerToRectify);
    call_unit(ADDR_ARB_OWNER_PUBLIC, &cd, value, u64::MAX)
}

#[cfg(feature = "alloc")]
pub fn call_arb_owner_public_get_all_chain_owners() -> Option<Vec<[u8; 20]>> {
    let cd = make_fn_arb_owner_public_get_all_chain_owners();
    let (rc, v) = static_call_vec(ADDR_ARB_OWNER_PUBLIC, &cd, u64::MAX, 0);
    if rc { decode_addr_vec(&v) } else { None }
}

pub fn call_arb_owner_public_get_native_token_management_from() -> u64 {
    let cd = make_fn_arb_owner_public_get_native_token_management_from();
    u64_from(static_word(ADDR_ARB_OWNER_PUBLIC, &cd))
}

pub fn call_arb_owner_public_is_native_token_owner(addr: [u8; 20]) -> bool {
...
pub fn call_arb_owner_public_get_transaction_filtering_from() -> u64 {
    let cd = make_fn_arb_owner_public_get_transaction_filtering_from();
    u64_from(static_word(ADDR_ARB_OWNER_PUBLIC, &cd))
}

pub fn call_arb_owner_public_is_transaction_filterer(filterer: [u8; 20]) -> bool {
...
pub fn call_arb_owner_public_get_filtered_funds_recipient() -> [u8; 20] {
    let cd = make_fn_arb_owner_public_get_filtered_funds_recipient();
    addr_from(static_word(ADDR_ARB_OWNER_PUBLIC, &cd))
}

pub fn call_arb_owner_public_get_network_fee_account() -> [u8; 20] {
    let cd = make_fn_arb_owner_public_get_network_fee_account();
    addr_from(static_word(ADDR_ARB_OWNER_PUBLIC, &cd))
}

pub fn call_arb_owner_public_get_infra_fee_account() -> [u8; 20] {
    let cd = make_fn_arb_owner_public_get_infra_fee_account();
    addr_from(static_word(ADDR_ARB_OWNER_PUBLIC, &cd))
}

pub fn call_arb_owner_public_get_brotli_compression_level() -> u64 {
    let cd = make_fn_arb_owner_public_get_brotli_compression_level();
    u64_from(static_word(ADDR_ARB_OWNER_PUBLIC, &cd))
}

pub fn call_arb_owner_public_get_parent_gas_floor_per_token() -> u64 {
    let cd = make_fn_arb_owner_public_get_parent_gas_floor_per_token();
    u64_from(static_word(ADDR_ARB_OWNER_PUBLIC, &cd))
}

pub fn call_arb_owner_public_get_scheduled_upgrade() -> (u64, u64) {
    let cd = make_fn_arb_owner_public_get_scheduled_upgrade();
    let ws = static_words::<2, { 32 * 2 }>(ADDR_ARB_OWNER_PUBLIC, &cd);
    (u64_from(ws[0]), u64_from(ws[1]))
}

pub fn call_arb_owner_public_is_calldata_price_increase_enabled() -> bool {
    let cd = make_fn_arb_owner_public_is_calldata_price_increase_enabled();
    u8_from(static_word(ADDR_ARB_OWNER_PUBLIC, &cd)) != 0
}

pub fn call_arb_owner_public_get_collect_tips() -> bool {
    let cd = make_fn_arb_owner_public_get_collect_tips();
    u8_from(static_word(ADDR_ARB_OWNER_PUBLIC, &cd)) != 0
}

pub fn call_arb_owner_public_get_max_stylus_contract_fragments() -> u8 {
    let cd = make_fn_arb_owner_public_get_max_stylus_contract_fragments();
    u8_from(static_word(ADDR_ARB_OWNER_PUBLIC, &cd))
}

pub fn call_arb_gas_info_get_prices_in_wei_with_aggregator(aggregator: [u8; 20]) -> [U; 6] {
    let cd = make_fn_arb_gas_info_get_prices_in_wei_with_aggregator(aggregator);
    static_words::<6, { 32 * 6 }>(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_prices_in_wei() -> [U; 6] {
    let cd = make_fn_arb_gas_info_get_prices_in_wei();
    static_words::<6, { 32 * 6 }>(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_prices_in_arb_gas_with_aggregator(aggregator: [u8; 20]) -> [U; 3] {
    let cd = make_fn_arb_gas_info_get_prices_in_arb_gas_with_aggregator(aggregator);
    static_words::<3, { 32 * 3 }>(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_prices_in_arb_gas() -> [U; 3] {
    let cd = make_fn_arb_gas_info_get_prices_in_arb_gas();
    static_words::<3, { 32 * 3 }>(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_gas_accounting_params() -> [U; 3] {
    let cd = make_fn_arb_gas_info_get_gas_accounting_params();
    static_words::<3, { 32 * 3 }>(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_max_tx_gas_limit() -> U {
    let cd = make_fn_arb_gas_info_get_max_tx_gas_limit();
    static_word(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_minimum_gas_price() -> U {
    let cd = make_fn_arb_gas_info_get_minimum_gas_price();
    static_word(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_l1_base_fee_estimate() -> U {
    let cd = make_fn_arb_gas_info_get_l1_base_fee_estimate();
    static_word(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_l1_base_fee_estimate_inertia() -> u64 {
    let cd = make_fn_arb_gas_info_get_l1_base_fee_estimate_inertia();
    u64_from(static_word(ADDR_ARB_GAS_INFO, &cd))
}

pub fn call_arb_gas_info_get_l1_reward_rate() -> u64 {
    let cd = make_fn_arb_gas_info_get_l1_reward_rate();
    u64_from(static_word(ADDR_ARB_GAS_INFO, &cd))
}

pub fn call_arb_gas_info_get_l1_reward_recipient() -> [u8; 20] {
    let cd = make_fn_arb_gas_info_get_l1_reward_recipient();
    addr_from(static_word(ADDR_ARB_GAS_INFO, &cd))
}

pub fn call_arb_gas_info_get_l1_gas_price_estimate() -> U {
    let cd = make_fn_arb_gas_info_get_l1_gas_price_estimate();
    static_word(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_current_tx_l1_gas_fees() -> U {
    let cd = make_fn_arb_gas_info_get_current_tx_l1_gas_fees();
    static_word(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_gas_backlog() -> u64 {
    let cd = make_fn_arb_gas_info_get_gas_backlog();
    u64_from(static_word(ADDR_ARB_GAS_INFO, &cd))
}

pub fn call_arb_gas_info_get_pricing_inertia() -> u64 {
    let cd = make_fn_arb_gas_info_get_pricing_inertia();
    u64_from(static_word(ADDR_ARB_GAS_INFO, &cd))
}

pub fn call_arb_gas_info_get_gas_backlog_tolerance() -> u64 {
    let cd = make_fn_arb_gas_info_get_gas_backlog_tolerance();
    u64_from(static_word(ADDR_ARB_GAS_INFO, &cd))
}

pub fn call_arb_gas_info_get_l1_pricing_surplus() -> U {
    let cd = make_fn_arb_gas_info_get_l1_pricing_surplus();
    static_word(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_per_batch_gas_charge() -> u64 {
    let cd = make_fn_arb_gas_info_get_per_batch_gas_charge();
    u64_from(static_word(ADDR_ARB_GAS_INFO, &cd))
}

pub fn call_arb_gas_info_get_amortized_cost_cap_bips() -> u64 {
    let cd = make_fn_arb_gas_info_get_amortized_cost_cap_bips();
    u64_from(static_word(ADDR_ARB_GAS_INFO, &cd))
}

pub fn call_arb_gas_info_get_l1_fees_available() -> U {
    let cd = make_fn_arb_gas_info_get_l1_fees_available();
    static_word(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_l1_pricing_equilibration_units() -> U {
    let cd = make_fn_arb_gas_info_get_l1_pricing_equilibration_units();
    static_word(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_last_l1_pricing_update_time() -> u64 {
    let cd = make_fn_arb_gas_info_get_last_l1_pricing_update_time();
    u64_from(static_word(ADDR_ARB_GAS_INFO, &cd))
}

pub fn call_arb_gas_info_get_l1_pricing_funds_due_for_rewards() -> U {
    let cd = make_fn_arb_gas_info_get_l1_pricing_funds_due_for_rewards();
    static_word(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_l1_pricing_units_since_update() -> u64 {
    let cd = make_fn_arb_gas_info_get_l1_pricing_units_since_update();
    u64_from(static_word(ADDR_ARB_GAS_INFO, &cd))
}

pub fn call_arb_gas_info_get_last_l1_pricing_surplus() -> U {
    let cd = make_fn_arb_gas_info_get_last_l1_pricing_surplus();
    static_word(ADDR_ARB_GAS_INFO, &cd)
}

pub fn call_arb_gas_info_get_max_block_gas_limit() -> u64 {
...
pub fn call_arb_gas_info_get_gas_pricing_constraints() -> Option<Vec<[U; 3]>> {
    ...
    let (rc, v) = static_call_vec(ADDR_ARB_GAS_INFO, &cd, u64::MAX, 0);
...
pub fn call_arb_aggregator_get_preferred_aggregator(addr: [u8; 20]) -> ([u8; 20], bool) {
    let cd = make_fn_arb_aggregator_get_preferred_aggregator(addr);
    let ws = static_words::<2, { 32 * 2 }>(ADDR_ARB_AGGREGATOR, &cd);
    (addr_from(ws[0]), u8_from(ws[1]) != 0)
}

pub fn call_arb_aggregator_get_default_aggregator() -> [u8; 20] {
    let cd = make_fn_arb_aggregator_get_default_aggregator();
    addr_from(static_word(ADDR_ARB_AGGREGATOR, &cd))
}

#[cfg(feature = "alloc")]
pub fn call_arb_aggregator_get_batch_posters() -> Option<Vec<[u8; 20]>> {
    let cd = make_fn_arb_aggregator_get_batch_posters();
    let (rc, v) = static_call_vec(ADDR_ARB_AGGREGATOR, &cd, u64::MAX, 0);
    if rc { decode_addr_vec(&v) } else { None }
}

pub fn call_arb_aggregator_add_batch_poster(newBatchPoster: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_aggregator_add_batch_poster(newBatchPoster);
    call_unit(ADDR_ARB_AGGREGATOR, &cd, value, u64::MAX)
}

pub fn call_arb_aggregator_get_fee_collector(batchPoster: [u8; 20]) -> [u8; 20] {
    let cd = make_fn_arb_aggregator_get_fee_collector(batchPoster);
    addr_from(static_word(ADDR_ARB_AGGREGATOR, &cd))
}

pub fn call_arb_aggregator_set_fee_collector(
    batchPoster: [u8; 20],
    newFeeCollector: [u8; 20],
    value: &U,
) -> bool {
...
pub fn call_arb_aggregator_get_tx_base_fee(aggregator: [u8; 20]) -> U {
    let cd = make_fn_arb_aggregator_get_tx_base_fee(aggregator);
    static_word(ADDR_ARB_AGGREGATOR, &cd)
}

pub fn call_arb_aggregator_set_tx_base_fee(aggregator: [u8; 20], feeInL1Gas: U, value: &U) -> bool {
    let cd = make_fn_arb_aggregator_set_tx_base_fee(aggregator, feeInL1Gas);
    call_unit(ADDR_ARB_AGGREGATOR, &cd, value, u64::MAX)
}

pub fn call_arb_retryable_tx_redeem(ticketId: [u8; 32], value: &U) -> U {
    let cd = make_fn_arb_retryable_tx_redeem(ticketId);
    call_word(ADDR_ARB_RETRYABLE_TX, &cd, value)
}

pub fn call_arb_retryable_tx_get_lifetime() -> U {
    let cd = make_fn_arb_retryable_tx_get_lifetime();
    static_word(ADDR_ARB_RETRYABLE_TX, &cd)
}

pub fn call_arb_retryable_tx_get_timeout(ticketId: [u8; 32]) -> U {
    let cd = make_fn_arb_retryable_tx_get_timeout(ticketId);
    static_word(ADDR_ARB_RETRYABLE_TX, &cd)
}

pub fn call_arb_retryable_tx_keepalive(ticketId: [u8; 32], value: &U) -> U {
    let cd = make_fn_arb_retryable_tx_keepalive(ticketId);
    call_word(ADDR_ARB_RETRYABLE_TX, &cd, value)
}

pub fn call_arb_retryable_tx_get_beneficiary(ticketId: [u8; 32]) -> [u8; 20] {
    let cd = make_fn_arb_retryable_tx_get_beneficiary(ticketId);
    addr_from(static_word(ADDR_ARB_RETRYABLE_TX, &cd))
}

pub fn call_arb_retryable_tx_cancel(ticketId: [u8; 32], value: &U) -> bool {
    let cd = make_fn_arb_retryable_tx_cancel(ticketId);
    call_unit(ADDR_ARB_RETRYABLE_TX, &cd, value, u64::MAX)
}

pub fn call_arb_retryable_tx_get_current_redeemer() -> [u8; 20] {
    let cd = make_fn_arb_retryable_tx_get_current_redeemer();
    addr_from(static_word(ADDR_ARB_RETRYABLE_TX, &cd))
}

pub fn call_arb_retryable_tx_submit_retryable<
    const N: usize,
    const PAD: usize,
    const TOTAL: usize,
>(
    ...
    let cd = make_fn_arb_retryable_tx_submit_retryable::<N, PAD, TOTAL>(
        requestId,
        l1BaseFee,
        deposit,
        callvalue,
...
pub fn call_arb_statistics_get_stats() -> [U; 6] {
    let cd = make_fn_arb_statistics_get_stats();
    static_words::<6, { 32 * 6 }>(ADDR_ARB_STATISTICS, &cd)
}

pub fn call_arb_owner_add_chain_owner(newOwner: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_owner_add_chain_owner(newOwner);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_remove_chain_owner(owner: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_owner_remove_chain_owner(owner);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_is_chain_owner(addr: [u8; 20]) -> bool {
    let cd = make_fn_arb_owner_is_chain_owner(addr);
    u8_from(static_word(ADDR_ARB_OWNER, &cd)) != 0
}

#[cfg(feature = "alloc")]
pub fn call_arb_owner_get_all_chain_owners() -> Option<Vec<[u8; 20]>> {
    let cd = make_fn_arb_owner_get_all_chain_owners();
    let (rc, v) = static_call_vec(ADDR_ARB_OWNER, &cd, u64::MAX, 0);
    if rc { decode_addr_vec(&v) } else { None }
}

pub fn call_arb_owner_set_native_token_management_from(newTimestamp: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_native_token_management_from(newTimestamp);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_transaction_filtering_from(newTimestamp: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_transaction_filtering_from(newTimestamp);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_add_native_token_owner(newOwner: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_owner_add_native_token_owner(newOwner);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_remove_native_token_owner(owner: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_owner_remove_native_token_owner(owner);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_is_native_token_owner(addr: [u8; 20]) -> bool {
...
pub fn call_arb_owner_add_transaction_filterer(newFilterer: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_owner_add_transaction_filterer(newFilterer);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_remove_transaction_filterer(filterer: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_owner_remove_transaction_filterer(filterer);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_is_transaction_filterer(addr: [u8; 20]) -> bool {
...
pub fn call_arb_owner_set_filtered_funds_recipient(receiver: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_owner_set_filtered_funds_recipient(receiver);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_get_filtered_funds_recipient() -> [u8; 20] {
    let cd = make_fn_arb_owner_get_filtered_funds_recipient();
    addr_from(static_word(ADDR_ARB_OWNER, &cd))
}

pub fn call_arb_owner_set_l1_base_fee_estimate_inertia(inertia: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_l1_base_fee_estimate_inertia(inertia);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_l2_base_fee(priceInWei: U, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_l2_base_fee(priceInWei);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_minimum_l2_base_fee(priceInWei: U, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_minimum_l2_base_fee(priceInWei);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_speed_limit(speedLimit: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_speed_limit(speedLimit);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_max_tx_gas_limit(maxTxGasLimit: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_max_tx_gas_limit(maxTxGasLimit);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_max_block_gas_limit(maxBlockGasLimit: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_max_block_gas_limit(maxBlockGasLimit);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_l2_gas_pricing_inertia(inertia: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_l2_gas_pricing_inertia(inertia);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_l2_gas_backlog_tolerance(tolerance: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_l2_gas_backlog_tolerance(tolerance);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_get_network_fee_account() -> [u8; 20] {
    let cd = make_fn_arb_owner_get_network_fee_account();
    addr_from(static_word(ADDR_ARB_OWNER, &cd))
}

pub fn call_arb_owner_get_infra_fee_account() -> [u8; 20] {
    let cd = make_fn_arb_owner_get_infra_fee_account();
    addr_from(static_word(ADDR_ARB_OWNER, &cd))
}

pub fn call_arb_owner_set_network_fee_account(newNetworkFeeAccount: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_owner_set_network_fee_account(newNetworkFeeAccount);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_infra_fee_account(newInfraFeeAccount: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_owner_set_infra_fee_account(newInfraFeeAccount);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_schedule_arb_o_s_upgrade(newVersion: u64, timestamp: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_schedule_arb_o_s_upgrade(newVersion, timestamp);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_l1_pricing_equilibration_units(units: U, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_l1_pricing_equilibration_units(units);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_l1_pricing_inertia(inertia: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_l1_pricing_inertia(inertia);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_l1_pricing_reward_recipient(recipient: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_owner_set_l1_pricing_reward_recipient(recipient);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_l1_pricing_reward_rate(recipientRate: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_l1_pricing_reward_rate(recipientRate);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_l1_price_per_unit(pricePerUnit: U, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_l1_price_per_unit(pricePerUnit);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_parent_gas_floor_per_token(floorPerToken: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_parent_gas_floor_per_token(floorPerToken);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_per_batch_gas_charge(charge: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_per_batch_gas_charge(charge);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_brotli_compression_level(level: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_brotli_compression_level(level);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_amortized_cost_cap_bips(capBips: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_amortized_cost_cap_bips(capBips);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_release_l1_pricer_surplus_funds(surplusToRelease: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_release_l1_pricer_surplus_funds(surplusToRelease);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_ink_price(price: u32, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_ink_price(price);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_wasm_max_stack_depth(depth: u32, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_wasm_max_stack_depth(depth);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_wasm_free_pages(amount: u16, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_wasm_free_pages(amount);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_wasm_page_gas(amount: u16, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_wasm_page_gas(amount);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_wasm_page_limit(limit: u32, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_wasm_page_limit(limit);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_wasm_max_size(size: u32, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_wasm_max_size(size);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_wasm_min_init_gas(gas: u8, cached: u16, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_wasm_min_init_gas(gas, cached);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_wasm_init_cost_scalar(percent: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_wasm_init_cost_scalar(percent);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_wasm_expiry_days(days: u16, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_wasm_expiry_days(days);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_wasm_keepalive_days(days: u16, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_wasm_keepalive_days(days);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_wasm_block_cache_size(size: u16, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_wasm_block_cache_size(size);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_add_wasm_cache_manager(manager: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_owner_add_wasm_cache_manager(manager);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_remove_wasm_cache_manager(manager: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_owner_remove_wasm_cache_manager(manager);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_chain_config<const N: usize, const PAD: usize, const TOTAL: usize>(
    buf: &[u8; N],
    value: &U,
) -> bool {
...
pub fn call_arb_owner_set_calldata_price_increase(enable: bool, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_calldata_price_increase(enable);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_gas_backlog(backlog: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_gas_backlog(backlog);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_gas_pricing_constraints<const N: usize, const TOTAL: usize>(
    tail: &[u8; N],
    value: &U,
) -> bool {
...
pub fn call_arb_owner_set_multi_gas_pricing_constraints<const N: usize, const TOTAL: usize>(
    tail: &[u8; N],
    value: &U,
) -> bool {
...
pub fn call_arb_owner_set_collect_tips(collectTips: bool, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_collect_tips(collectTips);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_max_stylus_contract_fragments(maxFragments: u8, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_max_stylus_contract_fragments(maxFragments);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_owner_set_wasm_activation_gas(gas: u64, value: &U) -> bool {
    let cd = make_fn_arb_owner_set_wasm_activation_gas(gas);
    call_unit(ADDR_ARB_OWNER, &cd, value, u64::MAX)
}

pub fn call_arb_wasm_activate_program(program: [u8; 20], value: &U) -> (u16, U) {
    let cd = make_fn_arb_wasm_activate_program(program);
    let ws = call_words::<2, { 32 * 2 }>(ADDR_ARB_WASM, &cd, value);
    (u16_from(ws[0]), ws[1])
}

pub fn call_arb_wasm_stylus_version() -> u16 {
    let cd = make_fn_arb_wasm_stylus_version();
    u16_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_codehash_version(codehash: [u8; 32]) -> u16 {
    let cd = make_fn_arb_wasm_codehash_version(codehash);
    u16_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_codehash_keepalive(codehash: [u8; 32], value: &U) -> bool {
    let cd = make_fn_arb_wasm_codehash_keepalive(codehash);
    call_unit(ADDR_ARB_WASM, &cd, value, u64::MAX)
}

pub fn call_arb_wasm_codehash_asm_size(codehash: [u8; 32]) -> u32 {
    let cd = make_fn_arb_wasm_codehash_asm_size(codehash);
    u32_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_program_version(program: [u8; 20]) -> u16 {
    let cd = make_fn_arb_wasm_program_version(program);
    u16_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_program_init_gas(program: [u8; 20]) -> (u64, u64) {
    let cd = make_fn_arb_wasm_program_init_gas(program);
    let ws = static_words::<2, { 32 * 2 }>(ADDR_ARB_WASM, &cd);
    (u64_from(ws[0]), u64_from(ws[1]))
}

pub fn call_arb_wasm_program_memory_footprint(program: [u8; 20]) -> u16 {
    let cd = make_fn_arb_wasm_program_memory_footprint(program);
    u16_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_program_time_left(program: [u8; 20]) -> u64 {
    let cd = make_fn_arb_wasm_program_time_left(program);
    u64_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_ink_price() -> u32 {
    let cd = make_fn_arb_wasm_ink_price();
    u32_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_max_stack_depth() -> u32 {
    let cd = make_fn_arb_wasm_max_stack_depth();
    u32_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_free_pages() -> u16 {
    let cd = make_fn_arb_wasm_free_pages();
    u16_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_page_gas() -> u16 {
    let cd = make_fn_arb_wasm_page_gas();
    u16_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_page_ramp() -> u64 {
    let cd = make_fn_arb_wasm_page_ramp();
    u64_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_page_limit() -> u16 {
    let cd = make_fn_arb_wasm_page_limit();
    u16_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_min_init_gas() -> (u64, u64) {
    let cd = make_fn_arb_wasm_min_init_gas();
    let ws = static_words::<2, { 32 * 2 }>(ADDR_ARB_WASM, &cd);
    (u64_from(ws[0]), u64_from(ws[1]))
}

pub fn call_arb_wasm_init_cost_scalar() -> u64 {
    let cd = make_fn_arb_wasm_init_cost_scalar();
    u64_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_expiry_days() -> u16 {
    let cd = make_fn_arb_wasm_expiry_days();
    u16_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_keepalive_days() -> u16 {
    let cd = make_fn_arb_wasm_keepalive_days();
    u16_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_block_cache_size() -> u16 {
    let cd = make_fn_arb_wasm_block_cache_size();
    u16_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_activation_gas() -> u64 {
    let cd = make_fn_arb_wasm_activation_gas();
    u64_from(static_word(ADDR_ARB_WASM, &cd))
}

pub fn call_arb_wasm_cache_is_cache_manager(manager: [u8; 20]) -> bool {
    let cd = make_fn_arb_wasm_cache_is_cache_manager(manager);
    u8_from(static_word(ADDR_ARB_WASM_CACHE, &cd)) != 0
}

#[cfg(feature = "alloc")]
pub fn call_arb_wasm_cache_all_cache_managers() -> Option<Vec<[u8; 20]>> {
    let cd = make_fn_arb_wasm_cache_all_cache_managers();
    let (rc, v) = static_call_vec(ADDR_ARB_WASM_CACHE, &cd, u64::MAX, 0);
    if rc { decode_addr_vec(&v) } else { None }
}

pub fn call_arb_wasm_cache_cache_codehash(codehash: [u8; 32], value: &U) -> bool {
    let cd = make_fn_arb_wasm_cache_cache_codehash(codehash);
    call_unit(ADDR_ARB_WASM_CACHE, &cd, value, u64::MAX)
}

pub fn call_arb_wasm_cache_cache_program(addr: [u8; 20], value: &U) -> bool {
    let cd = make_fn_arb_wasm_cache_cache_program(addr);
    call_unit(ADDR_ARB_WASM_CACHE, &cd, value, u64::MAX)
}

pub fn call_arb_wasm_cache_evict_codehash(codehash: [u8; 32], value: &U) -> bool {
    let cd = make_fn_arb_wasm_cache_evict_codehash(codehash);
    call_unit(ADDR_ARB_WASM_CACHE, &cd, value, u64::MAX)
}

pub fn call_arb_wasm_cache_codehash_is_cached(codehash: [u8; 32]) -> bool {
    let cd = make_fn_arb_wasm_cache_codehash_is_cached(codehash);
    u8_from(static_word(ADDR_ARB_WASM_CACHE, &cd)) != 0
}

pub fn call_arb_native_token_manager_mint_native_token(amount: U, value: &U) -> bool {
    let cd = make_fn_arb_native_token_manager_mint_native_token(amount);
    call_unit(ADDR_ARB_NATIVE_TOKEN_MANAGER, &cd, value, u64::MAX)
}

pub fn call_arb_native_token_manager_burn_native_token(amount: U, value: &U) -> bool {
    let cd = make_fn_arb_native_token_manager_burn_native_token(amount);
    call_unit(ADDR_ARB_NATIVE_TOKEN_MANAGER, &cd, value, u64::MAX)
}

pub fn call_arb_debug_become_chain_owner(value: &U) -> Option<()> {
    let cd = make_fn_arb_debug_become_chain_owner();
    call_unit(ADDR_ARB_DEBUG, &cd, value, u64::MAX).then_some(())
}

#[cfg(feature = "alloc")]
pub fn call_arb_debug_overwrite_contract_code<
    const N: usize,
    const PAD: usize,
    const TOTAL: usize,
>(
    ...
    let (rc, v) = call_vec(ADDR_ARB_DEBUG, &cd, value, u64::MAX, 0);
    rc.then_some(v)
}
...
pub fn call_arb_debug_events_view() -> Option<()> {
    let cd = make_fn_arb_debug_events_view();
    static_call_unit(ADDR_ARB_DEBUG, &cd, u64::MAX).then_some(())
}

pub fn call_arb_debug_custom_revert(number: u64, value: &U) -> Option<()> {
    let cd = make_fn_arb_debug_custom_revert(number);
    call_unit(ADDR_ARB_DEBUG, &cd, value, u64::MAX).then_some(())
}

pub fn call_arb_debug_panic(value: &U) -> Option<()> {
    let cd = make_fn_arb_debug_panic();
    call_unit(ADDR_ARB_DEBUG, &cd, value, u64::MAX).then_some(())
}

pub fn call_arb_debug_legacy_error(value: &U) -> Option<()> {
