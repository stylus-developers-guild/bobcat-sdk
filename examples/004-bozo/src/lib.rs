...
// The entrypoint dispatch selectors are generated by the `#[evm_entrypoint]`
// derive on `Entry` below. The only hand-written selector we keep is the
// reentrancy key for `state_play`, which the guard needs as a raw 4-byte word.
...
fn state_upgrade(new_impl: Address) -> usize {
    core::assert_eq!(storage_load(&SLOT_ADMIN), U::from(msg_sender()));
...
fn state_change_admin(new_admin: Address) -> usize {
    ...
    core::assert_eq!(last_admin, U::from(msg_sender()));
...
pub enum Entry {
    ...
    Initialise(EvmCdAddress, EvmCdAddress),
    Play(U, EvmCdAddress, [u8; 32], U),
    DistributeRewards(U, EvmCdAddress, U),
    Upgrade(EvmCdAddress),
    ChangeAdmin(EvmCdAddress),
    OwnerCollectFees,
    DaoCollectFees,
}
...
pub fn user_entrypoint(args_len: usize) -> usize {
    flush_guard(|| match read_cd::<Entry>(args_len) {
        // View functions:
        Entry::Deadline => view_deadline(),
        Entry::EpochDeadline(epoch) => view_deadline_epoch(epoch),
        Entry::PoolSize => view_pool_size(),
        Entry::PoolAsset => view_pool_asset(),
        Entry::LastBettorAmount => view_last_bettor_amount(),
        Entry::LastBettorAddress => view_last_bettor_address(),
        Entry::MinDeposit => view_min_deposit(),
        Entry::PlayerCount => view_player_count(),
        Entry::TicketCount => view_ticket_count(),
        Entry::CurrentEpoch => view_current_epoch(),
        Entry::WasEpochCollected(_) => view_was_epoch_collected(),
        // Side effect generating functions:
        Entry::Initialise(admin, asset) => state_init(admin.into_array(), asset.into_array()),
        Entry::Play(amt, recipient, comment, epoch) => reentrancy_guard_sel(&SEL_PLAY, || {
            let comment = &U::from(comment);
            let epoch = &epoch;
            state_play(amt, recipient.into_array(), comment, epoch)
        }),
        Entry::DistributeRewards(epoch, _recipient, rng) => state_distribute_rewards(&epoch, &rng),
        Entry::Upgrade(new_impl) => state_upgrade(new_impl.into_array()),
        Entry::ChangeAdmin(new_admin) => state_change_admin(new_admin.into_array()),
        Entry::OwnerCollectFees => state_owner_collect_fees(),
        Entry::DaoCollectFees => state_dao_collect_fees(),
    })
}
