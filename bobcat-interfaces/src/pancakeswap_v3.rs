//! Calldata builders for PancakeSwap V3's canonical `ISwapRouter`.
//!
//! PancakeSwap V3 is a fork of Uniswap V3; the router exposes the same
//! `exactInputSingle` and `exactOutputSingle` entrypoints with the same
//! struct layout. This module supports the deadline-bearing, single-hop
//! swap functions only.
...
/// Encode an exact-input swap through one PancakeSwap V3 pool.
...
/// Encode an exact-output swap through one PancakeSwap V3 pool.
#[allow(clippy::too_many_arguments)]
